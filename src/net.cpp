...
void ThreadSocketHandler()
{
    ...
    while (true)
    {
        ...
        BOOST_FOREACH(SOCKET hListenSocket, vhListenSocket) {
            FD_SET(hListenSocket, &pwcetRecv);
                    ...
                    continue;
                {
                    ...
                    if (lockSend) {
                        ...
                        if (!pnode->vSendMsg.empty())
                            FD_SET(pnode->hSocket, &pwcetSend);
                        else
                            FD_SET(pnode->hSocket, &pwcetRecv);
                        FD_SET(pnode->hSocket, &pwcetError);
        ...
        int nSelect = select(have_pwc ? hSocketMax + 1 : 0,
                             &pwcetRecv, &pwcetSend, &pwcetError, &timeout);
        boost::this_thread::interruption_point();
        ...
        //
        BOOST_FOREACH(SOCKET hListenSocket, vhListenSocket)
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &pwcetRecv))
        {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            if (pnode->hSocket == INVALID_SOCKET)
                continue;
            if (FD_ISSET(pnode->hSocket, &pwcetRecv) || FD_ISSET(pnode->hSocket, &pwcetError))
            {
                TRY_LOCK(pnode->cs_vRecvMsg, lockRecv);
                if (lockRecv)
                {
            ...
            if (pnode->hSocket == INVALID_SOCKET)
                continue;
            if (FD_ISSET(pnode->hSocket, &pwcetSend))
            {
                TRY_LOCK(pnode->cs_vSend, lockSend);
                if (lockSend)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Rel " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Rel is probably already running."), addrBind.ToString());
        else
